...
int main() {
    ...
    while (window.isOpen()) {
        ...
        {
            //        for (unsigned int i = 0; i < mapka["potwory"].size(); i++) {
            //            Potwor &as_potwor = dynamic_cast<Potwor &>(*mapka["potwory"][i]);
            //            as_potwor.Dzialaj(elapsed.asSeconds());
            //        }
            //        for (unsigned int i = 0; i < mapka["bagno"].size(); i++) {
            //            Bagno &as_bagno = dynamic_cast<Bagno &>(*mapka["bagno"][i]);
