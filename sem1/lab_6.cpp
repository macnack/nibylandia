...
public:
  ...
  element(const std::string arg)
  {
    nazwa = arg;
    napiecie = czestotliwosc = 1.0;
    prad = moc = 0.0;
  }
...
void obwod(element *e1, element *e2)
{
  ...
  if ((pradMatch && !napiecieMatch) || (!pradMatch && napiecieMatch))
  {
    ...
    if (e1->napiecie == e2->napiecie)
    {
      cout << "###########    sa poloczone rownolegle     ###########" << endl
           << endl;
    }
    if (e1->prad == e2->prad)
    {
      cout << "###########    sa polaczone szeregowo      ###########" << endl
           << endl;
    }
  }
  else
...
public:
  ...
  cewka(const std::string arg)
  {
    nazwa = arg;
    L = 1.0;
  }
  cewka(const std::string arg, double argL)
  {
    nazwa = arg;
    L = argL;
  }
...
public:
  ...
  rezystor(const std::string arg)
  {
    nazwa = arg;
    Om = 1.0;
  }
  rezystor(const std::string arg, double argOm)
  {
    nazwa = arg;
    Om = argOm;
  }
...
public:
  ...
  kondensator(const std::string arg)
  {
    nazwa = arg;
    C = 1.0;
  }
  kondensator(const std::string arg, double argC)
  {
    nazwa = arg;
    C = argC;
  }
